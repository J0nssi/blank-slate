...
const endGame = async (roomId) => {
  ...
  await updateDoc(roomRef, {
    ...
    currentRound: {
      timer: 10,            // Reset the timer for the next round
      wordPrompt: "",       // Clear the current word prompt
      wordsSubmitted: []    // Reset submitted words
    },
    usedWords: [],          // Clear used words
  });

  console.log("Game has ended and reset to the beginning");
};
